...
async def giahan(ctx):
    ...
    if role.position >= ctx.guild.me.top_role.position:
        await ctx.send(f"{ctx.author.mention}, role Gia Hạn có thứ tự cao hơn role của bot! Vui lòng điều chỉnh thứ tự role.")
        return
    ...
    if record and record["expiration_time"] > set_time:
        ...
        await ctx.send(f"{user.mention}, thời gian bạn được xem sếch được gia hạn thêm 50 ngày, còn {remaining_time}!")
        ...
        # Người dùng chưa có role Gia Hạn hoặc role đã hết hạn
        try:
            await user.add_roles(role)
        except Exception as e:
            logger.error(f"Lỗi khi cấp role Gia Hạn cho {user.id}: {e}")
            await ctx.send(f"{ctx.author.mention}, không thể cấp role Gia Hạn cho {user.mention} do lỗi: {str(e)}")
            return
        ...
        await ctx.send(f"{user.mention}, bạn đã được cấp thời gian xem sếch trong 50 ngày!")
        notification_channel = bot.get_channel(ROLE_NOTIFICATION_CHANNEL_ID)
        if notification_channel:
            await notification_channel.send(
                f"Gia hạn role cho {user.mention} vào {set_time.strftime('%H:%M %d/%m/%Y UTC')} "
...
async def rm(ctx):
    ...
    role = discord.utils.get(ctx.guild.roles, name="Gia Hạn")
    if not role:
        await ctx.send(f"{ctx.author.mention}, có lỗi rồi, liên hệ cho Admin nha!")
        return
    if role in user.roles:
        try:
            ...
            logger.error(f"Lỗi khi gỡ role Gia Hạn cho {user.id}: {e}")
            await ctx.send(f"{ctx.author.mention}, không thể gỡ role Gia Hạn khỏi {user.mention} do lỗi: {str(e)}")
    else:
        await ctx.send(f"{ctx.author.mention}, {user.mention} không có role Gia Hạn để gỡ!")
...
async def check(ctx, user: discord.Member = None):
    if user is None:
        ...
        if not has_role(ctx.author, ["Admin", "Mod", "Friendly Dev"]):
            await ctx.send(f"{ctx.author.mention}, bạn không có quyền kiểm tra thời gian của người khác! Hãy dùng `$check` để kiểm tra chính bạn.")
            return
    record = role_timers_collection.find_one({"user_id": user.id, "role_name": "Gia Hạn"})
...
async def log(ctx, user: discord.Member = None):
    if user is None:
        ...
        return
    history = role_history_collection.find({"user_id": user.id, "role_name": "Gia Hạn"}).sort("set_time", 1)
    history_list = []
